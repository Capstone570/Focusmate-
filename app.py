...
if "predict" in params:
    try:
        preds = ml_model.predict(np.array([[
            ...
            int(params.get("sleep", 7)),
            int(params.get("diff", 6)),
            int(params.get("energy", 8))
        ]]))[0]
...
<body class="relative flex flex-col justify-between items-center min-h-screen p-6">
    ...
    <div id="welcome-screen" class="{'relative z-10 flex flex-col items-center justify-center min-h-screen text-center w-full max-w-2xl mx-auto py-8' if start_page == '1' else 'hidden'}">
    ...
    <!-- SCREEN 2: WORKSPACE -->
    <div id="workspace-screen" class="{'relative z-10 w-full max-w-4xl mx-auto py-12' if start_page == '2' else 'hidden relative z-10 w-full max-w-4xl mx-auto py-12'}">
        ...
        <!-- RESULTS SECTION -->
        <div id="strategy-result" class="{'mt-10 space-y-6' if 'predict' in params else 'hidden mt-10 space-y-6'}">
            <div class="glass-card rounded-3xl p-8 border border-purple-500/30">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 text-center">
                    <div class="bg-slate-900/60 p-4 rounded-xl border border-slate-700">
                        ...
                        <span id="score-capacity" class="text-3xl font-extrabold text-purple-400">{capacity_val}%</span>
                    </div>
                    <div class="bg-slate-900/60 p-4 rounded-xl border border-slate-700">
                        <span class="block text-slate-400 text-sm">Sprint Duration</span>
                        <span id="score-sprint" class="text-3xl font-extrabold text-pink-400">{sprint_val} min</span>
                    </div>
                    <div class="bg-slate-900/60 p-4 rounded-xl border border-slate-700">
                        <span class="block text-slate-400 text-sm">Rest Interval</span>
                        <span id="score-rest" class="text-3xl font-extrabold text-sky-400">{rest_val} min</span>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-r from-slate-900 to-purple-950 p-6 rounded-2xl border border-purple-500/30 text-center mb-6">
                    ...
                    <div id="timer-display" class="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-purple-300 my-3">{sprint_val:02d}:00</div>
    ...
    <script>
        ...
        let timeRemaining = {sprint_val * 60};
        let isTimerRunning = false;
        let audioPlaying = false;
        let sprintsCompleted = 0;
        let currentSprintDuration = {sprint_val};
