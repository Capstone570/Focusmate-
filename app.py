...
if len(data) >= 15:
    ...
    try:
        ...
        accuracy_model = DecisionTreeClassifier(
            max_depth=5,
            random_state=42
        )
...
st.markdown(
    '<div class="main-title">FocusMate AI</div>',
    unsafe_allow_html=True
)
...
if st.button(
    ...
    if sleep >= 7:
        ...
        sleep_message = "Your sleep score suggests you may benefit from shorter sessions."
    ...
    st.write(
        f"**1. Prepare:** Set up everything needed for {task}."
    )

    st.write(
        f"**2. Focus:** Work continuously for {prediction} minutes."
    )
...
if "last_prediction" in st.session_state:
    ...
    st.write(
        "After completing your sprint, tell the AI how effective it was."
    )

    effectiveness = st.slider(
        "How effective was the sprint?",
        min_value=1,
        max_value=5,
        value=3
    )
    ...
    if st.button(
        ...
        if effectiveness >= 4:
            ...
            recommended_sprint = st.session_state[
                "last_prediction"
            ]
        ...
        st.info(
            f"The training dataset now contains {len(data)} examples."
        )
